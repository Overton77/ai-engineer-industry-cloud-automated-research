...
The Cloud environment is repository-managed at `.cursor/environment.json`. `install` runs `npm ci` from the committed lockfile, then verifies pinned CLIs and skill files. `start` creates `artifacts/` and `.firecrawl/`. Port `4848` is reserved for the Agent Browser dashboard.
